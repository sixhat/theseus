...
def rm_blank_lines(string):
    ...
    for i in range(len(out)-1,-1,-1):
        out[i]=out[i].strip(" \t")
        if len(out[i])<3: #Changed this to 3 to be consistent with the minimal char count for a word to be considered.
            out.pop(i)
            #pass
    return '\n'.join(out)
...
def process(infile):
    ...
    if sradius>0:
        for it in range(sradius,len(smooth)-sradius):
            smooth[it]=numpy.sum(score[(it-sradius):(it+sradius)])/(2*sradius+1)
